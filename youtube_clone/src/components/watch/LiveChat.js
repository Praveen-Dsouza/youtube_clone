...
const LiveChat = () => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            // API Polling
            dispatch(addMessage({name: generateRandomNames() , message: makeRandomMessage(20)}))
        }, 2000)
  ...
  return (
    <>
        ...
        <form className='w-full ml-2 p-2 border border-black flex' onSubmit={(e) => {
            ...
            <input className='w-80 px-2 border border-black' type='text' value={liveMessage} onChange={(e) => setLiveMessage(e.target.value)}/>
